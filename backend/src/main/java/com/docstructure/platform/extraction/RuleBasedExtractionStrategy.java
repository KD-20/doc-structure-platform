...
public class RuleBasedExtractionStrategy implements ExtractionStrategy {
    ...
    public ExtractionResult extract(ExtractionContext context) {
        ...
        // itself, but ExtractionService.triggerExtraction (the caller) is, and a nested
        ...
        RuleSetDefinition definition = ruleSetService.resolveDefinition(context.tenantId(), context.docType())
                .orElseThrow(() -> new ApiExceptions.NotFoundException(
                        "No active rule set for doc type '" + context.docType() + "'"));
