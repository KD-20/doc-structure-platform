...
## Deploying to a shared URL (Render + Supabase, both free)

For sharing a working instance with a team rather than running locally: `render.yaml` at the
repo root is a [Render Blueprint](https://render.com/docs/blueprint-spec) that deploys the same
Docker image `docker compose` builds locally, as a single free web service, wired to an
external Postgres instead of a `db` container (Render's free tier has no persistent disk to run
one on anyway). **Accepted tradeoff:** uploaded files' original bytes live on the container's
ephemeral disk and don't survive a restart/redeploy — but everything the UI actually shows
(extracted text, structured fields, search) lives in Postgres and is unaffected. Re-download of
the exact original file is the only thing that breaks.

1. **Create a free Postgres database** — [Supabase](https://supabase.com) is a good pick: free
   tier, pgvector already available. Create a project, then go to **Project Settings →
   Database** and copy the **direct connection** string (port `5432`) — not the pooled/
   transaction-mode one. This app runs `SELECT set_config('app.current_tenant_id', ?, true)` as
   the first statement of every tenant-scoped transaction (see `docs/DECISIONS.md §2`), which
   depends on the whole transaction staying on one physical connection; a transaction-mode
   pooler can hand out a different one mid-transaction and silently break tenant isolation.
2. **Generate two secrets locally:**
   ```
   openssl rand -base64 64   # PLATFORM_JWT_SECRET — needs to be this long, not just "long
                              # enough": JJWT picks HS256/384/512 by key length, and a shorter
                              # value can downgrade the algorithm or fail outright.
   ```
3. **Push this repo to GitHub** if it isn't already (Render deploys from a GitHub/GitLab repo).
4. In Render: **New → Blueprint**, point it at this repo. Render reads `render.yaml` and asks
   for the values marked `sync: false` in it:
   - `PLATFORM_JWT_SECRET` — the value from step 2.
   - `SPRING_DATASOURCE_URL` — `jdbc:postgresql://<supabase-host>:5432/postgres` (build this
     from the connection string in step 1 — same host/port/database, JDBC form instead of
     `postgres://user:pass@host/db`).
   - `SPRING_FLYWAY_USER` / `SPRING_FLYWAY_PASSWORD` — Supabase's project role is `postgres`
     and the password you set when creating the project. This is the one connection in the
     whole setup that needs elevated rights (`CREATE EXTENSION` / `CREATE ROLE`, run once per
     deploy by Flyway) — deliberately separate from the app's own everyday `app_user`
     connection, same split as local dev (`docs/DECISIONS.md §2a`).
   - `PLATFORM_BOOTSTRAP_ADMIN_PASSWORD` and `SPRING_DATASOURCE_PASSWORD` (the `app_user`
     Postgres password) are auto-generated by Render (`generateValue: true`) — no input needed,
     find the generated admin password afterward under the service's **Environment** tab.
5. Deploy. First boot runs Flyway (creates `app_user`, extensions, schema, RLS policies) against
   Supabase, then seeds the demo tenant/admin. Once healthy, Render gives you a
   `https://<your-service>.onrender.com` URL — that's what you share with the team.

**Known limitation of the free plan:** the service spins down after 15 minutes idle and takes
~30-60s to wake up on the next request — fine for a team checking in on a shared demo, not for
an always-instant experience. Upgrading off the free plan (or moving to a VM that stays up, e.g.
Oracle Cloud's Always Free tier) removes this if it becomes annoying.
